...
class Config:
    ...
    def __init__(
        self, id: int, file: str, config_path: str, server: str, client_id: int
    ) -> None:
        self.id = id
        self.file = file
        ...
        self.server = server
        self.server_line = (
            f", {lang.t('configs.server')}: [light_steel_blue]{server}[/]"
            if server != "-"
            else ""
        )
    ...
    def line(self) -> str:
        is_installed = os.path.exists(
            f"{self.client.path_dir}{self.config_path}{self.filename}"
        )
