...
function fetchVersions() {
    return new Promise((resolve) => {
        https.get(VERSION_MANIFEST_URL, { headers: { 'User-Agent': 'CollapseLoader-Script/1.0' } }, (res) => {
            let data = '';
            res.on('data', chunk => data += chunk);
            res.on('end', () => {
                try {
                    const json = JSON.parse(data);
                    const releases = json.versions.filter(v => v.type === 'release').map(v => v.id);
                    resolve(releases);
                } catch {
...
async function main() {
    ...
    const filePath = args.find(a => !a.startsWith('--') && !a.includes('\\') && !a.includes('/') && a.endsWith('.jar'));
    const version = args.find(a => /^\d+\.\d+/.test(a) && !a.includes('\\') && !a.includes('/'));
    const clientType = args.find(a => ['default', 'fabric', 'forge'].includes(a)) || 'default';
    const cdnRoot = args.find(a => a.includes('\\') || a.includes('/')) || 'E:\\hf-cdn';
    const extraFlags = args.filter(a => !a.startsWith('--') && !a.includes('\\') && !a.includes('/') && !a.endsWith('.jar') && !['default', 'fabric', 'forge'].includes(a) && !/^\d+\.\d+/.test(a));

    if (!filePath || !version) {
        console.error('Usage: node scripts/new_client.cjs <path-to.jar> <version> [default|fabric|forge] [flags...] [cdn-root]');
        console.error('       node scripts/new_client.cjs --versions                    # list available versions');
