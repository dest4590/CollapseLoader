...
const view = ref<"LOGIN" | "REGISTER" | "VERIFY" | "SUCCESS">(
...
const modalTitle = computed(() => {
    switch (view.value) {
        ...
        case "VERIFY":
            return t("auth.register.verification_required");
        default:
            return "";
    }
});
...
const handleResend = async () => {
    try {
        ...
        await apiPost(
            `${getApiBaseWithVersion()}/auth/resend-verification?email=${encodeURIComponent(pendingEmail.value)}`
        );
        addToast(
            t("common.verification_sent") || "Verification email resent!",
            "success"
        );
    } catch (e) {
