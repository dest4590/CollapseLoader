...
<template>
    <div class="relative filters-container">
        <button
            ...
            class="btn btn-ghost btn-sm ml-2 relative"
        >
            <Funnel class="w-4 h-4 mr-1" />
            {{ t("home.filters_title") }}
            <ChevronDown
                class="w-4 h-4 ml-1 transition-all duration-300"
                :class="{ 'rotate-180 ': showFiltersMenu }"
            />
            <span
            ...
            >
                <div class="flex flex-col gap-2 text-sm">
                    ...
                    <select class="select select-sm" v-model="localSortOrder">
                        <option value="asc">
                            {{ t("home.sort.order.asc") }}
                        </option>
                        <option value="desc">
                            {{ t("home.sort.order.desc") }}
                        </option>
                    </select>
                    ...
                    <label class="flex items-center gap-2">
                        <input
                            type="checkbox"
                            class="checkbox"
                            v-model="localFilters.installed"
                        />
