    ...
    <div class="space-y-4 flex flex-col h-full overflow-hidden">
        <div v-if="!selectedVersionsMod" class="flex items-center gap-2">
            <input v-model="searchQuery" @keyup.enter="handleSearch" type="text"
                :placeholder="t('mods.search_placeholder')" class="input input-bordered w-full" :disabled="isLoading" />
            <button @click="handleSearch" class="btn btn-primary" :disabled="isLoading">
                <Search class="w-4 h-4" />
            </button>
        </div>

        <div v-else class="flex items-center gap-4 border-b border-base-content/10 pb-2">
            <button @click="selectedVersionsMod = null" class="btn btn-sm btn-ghost btn-circle">
                <ChevronLeft class="w-4 h-4" />
            </button>
            <h3 class="font-bold truncate text-sm">{{ t('mods.versions_for', { mod: selectedVersionsMod.title }) }}</h3>
        </div>
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar pr-2 min-h-0">
            <div v-if="!selectedVersionsMod">
                ...
                <div v-else-if="mods.length === 0 && hasSearched" class="text-center py-8 text-base-content/60">
                    <p>{{ t('mods.no_results') }}</p>
                </div>

                <div v-else class="space-y-2">
                    <div v-for="mod in mods" :key="mod.project_id"
                        class="bg-base-200/50 p-3 rounded-lg flex items-center gap-3 hover:bg-base-200 transition-colors">
                        <img :src="mod.icon_url || 'https://cdn.modrinth.com/assets/unknown_server.png'"
                            :alt="mod.title" class="w-10 h-10 rounded shadow-sm object-cover" />
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-2">
                                <h4 class="font-bold text-sm truncate">{{ mod.title }}</h4>
                                <span class="text-[10px] bg-base-300 px-1.5 py-0.5 rounded text-base-content/60">{{
                                    mod.author }}</span>
                                <span v-if="isModInstalled(mod)"
                        ...
                        <button @click="showVersions(mod)" class="btn btn-sm btn-primary"
                            :disabled="installingMods.has(mod.project_id)">
                            <span v-if="installingMods.has(mod.project_id)"
            ...
            <div v-else class="space-y-2">
                <div v-if="isLoadingVersions" class="flex justify-center items-center h-40">
                    <span class="loading loading-spinner loading-lg text-primary"></span>
                </div>
                <div v-else-if="versions.length === 0" class="text-center py-8 text-base-content/60">
                    <p>{{ t('mods.no_compatible_version', { version: client.version }) }}</p>
                </div>
                <div v-else v-for="version in versions" :key="version.id"
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-2 font-bold text-sm">
                            ...
                            <span v-if="version.version_type === 'release'"
                                class="badge badge-success badge-xs">Release</span>
                            <span v-else-if="version.version_type === 'beta'"
                                class="badge badge-warning badge-xs">Beta</span>
                        </div>
                        <div class="text-[10px] text-base-content/50 mt-1">
                            {{ new Date(version.date_published).toLocaleDateString() }} • {{ version.loaders.join(', ')
                            }}
                        </div>
                    </div>
                    <button @click="installVersion(selectedVersionsMod, version)" class="btn btn-sm btn-primary">
                        {{ t('mods.install') }}
                    </button>
                </div>
            </div>
        ...
        <div class="flex justify-end gap-2 pt-2 border-t border-base-content/10">
            <button @click="emit('close')" class="btn btn-ghost">{{ t('common.close') }}</button>
        </div>
    </div>
</template>
...
import { ModrinthService, type ModrinthSearchResult, type ModrinthVersion } from '../../../services/modrinthService';
...
const fetchInstalledMods = async () => {
    try {
        installedModFiles.value = await invoke<string[]>('list_installed_mods', { id: props.client.id });
    } catch (e) {
...
const showVersions = async (mod: ModrinthSearchResult) => {
    ...
    try {
        ...
        versions.value = await ModrinthService.getModVersions(mod.slug, ['fabric'], [gameVersion]);
    } catch (e: any) {
...
const installVersion = async (mod: ModrinthSearchResult, version: ModrinthVersion) => {
