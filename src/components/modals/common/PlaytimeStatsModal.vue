<template>
    <div class="space-y-3">
        ...
        <transition-group name="list-fade" tag="div" appear>
            <div v-for="([clientName, stats], index) in entries"
                :key="clientName"
                :style="{'--i': index}"
                class="flex items-center gap-3 p-3 bg-base-300/40 rounded-lg mb-2">
                <span class="text-base-content/30 text-xs font-mono w-5 text-right shrink-0">{{ index+1 }}</span>
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <div class="flex items-center gap-3 mt-1">
                        <div class="flex-1 bg-base-content/10 rounded-full h-1.5">
                            <div class="bg-primary h-1.5 rounded-full transition-all" :style="{width:getPercent(stats.playtimeMinutes)+'%'}"></div>
                        </div>
                        <span class="text-xs text-base-content/50 shrink-0">{{ t('account.playtime_launches',{count:stats.launches}) }}</span>
                    </div>
                </div>
            </div>
        </transition-group>
...
const entries = computed(() => Object.entries(stats.value.clientStats).sort(([,a],[,b])=>b.playtimeMinutes-a.playtimeMinutes))

const maxMinutes = computed(() => Math.max(...entries.value.map(([,s])=>s.playtimeMinutes),1))

const getPercent = (minutes:number)=>Math.round((minutes/maxMinutes.value)*100)
