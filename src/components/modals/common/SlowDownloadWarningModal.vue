<template>
    <div class="space-y-3 mb-4">
        <p class="text-md text-base-content/80">
            {{
                isStalled
                    ? t("modals.slow_download.stalled_description")
                    : t("modals.slow_download.description")
            }}
        </p>
        ...
        <div v-if="!isStalled" class="bg-base-200 rounded-lg p-3">
            <div class="flex items-center justify-between">
                <span class="text-sm text-base-content/70">{{
                    t("modals.slow_download.current_speed")
                }}</span>
        ...
        <div v-else class="bg-error/10 border border-error/20 rounded-lg p-3">
            <div class="flex items-center gap-2">
                ...
                <span class="text-sm text-error">{{
                    t("modals.slow_download.no_progress")
                }}</span>
            </div>
        </div>
        ...
        >
            ...
            <li class="text-warning font-medium">
                {{ t("modals.slow_download.tip_zapret") }}
            </li>
        </ul>
    </div>
