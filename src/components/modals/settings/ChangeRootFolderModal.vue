...
const confirm = async () => {
    ...
    try {
        ...
        addToast(
            t("settings.change_root.failed", { error: String(e) }),
            "error"
        );
    } finally {
        busy.value = false;
        ...
        <div>
            <label class="label">
                <span class="label-text">{{
                    t("settings.change_root.target_folder")
                }}</span>
            </label>
            <div class="flex gap-2">
                ...
                />
                <button class="btn btn-secondary" @click="browse">
                    {{ t("common.browse") }}
                </button>
            </div>
            <p class="text-xs opacity-70 mt-2">
                {{ t("settings.change_root.hint") }}
            </p>
        </div>
        ...
        <div>
            ...
            <div class="join join-vertical sm:join-horizontal w-full">
                <input
                    ...
                    type="radio"
                    name="mode"
                    :aria-label="t('settings.change_root.mode_move')"
                    value="move"
                    v-model="mode"
                />
                <input
                    ...
                    type="radio"
                    name="mode"
                    :aria-label="t('settings.change_root.mode_wipe')"
                    value="wipe"
                    v-model="mode"
                />
            </div>
            <ul class="list-disc ml-5 mt-2 text-xs opacity-70">
                <li v-if="mode === 'move'">
                    {{ t("settings.change_root.mode_move_desc") }}
                </li>
