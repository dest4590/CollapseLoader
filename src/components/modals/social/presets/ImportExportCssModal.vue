    ...
    <div>
        <div v-if="mode === 'export'">
            ...
            <button
                ...
                @click="copyToClipboard(exportedCode)"
            >
                {{ t("theme.copy") }}
            </button>
        </div>
        <div v-else>
            ...
            <div v-if="importError" class="text-error text-sm mt-2">
                {{ importError }}
            </div>
        </div>
...
onMounted(async () => {
    if (props.mode === "export" && props.css) {
        try {
            exportedCode.value = await invoke("encode_base64", {
                input: props.css,
            });
        } catch (e) {
...
const importCss = async () => {
    importError.value = "";
    try {
        const decoded = await invoke("decode_base64", {
            input: importCode.value,
        });

        if (
            typeof decoded !== "string" ||
            /script|@import|url\(|expression|<|>|javascript:/i.test(decoded)
        ) {
