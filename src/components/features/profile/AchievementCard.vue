<template>
    <div class="card bg-base-100 shadow-sm border border-base-200 transition-all hover:shadow-md"
        ...
        <div class="card-body p-4 flex flex-row items-center gap-4">
            <div class="p-3 rounded-full bg-base-200 text-primary"
                :class="{ 'bg-base-300 text-base-content/50': locked }">
                <component :is="iconComponent" class="w-6 h-6" />
            </div>
            <div class="flex-1">
                <h3 class="font-bold text-sm" :class="{ 'text-base-content/50': locked }">
                    {{ title }}
                </h3>
                <p class="text-xs text-base-content/70">
                    {{ description }}
                </p>
                <p v-if="!locked && unlockedAt" class="text-[10px] text-primary mt-1">
