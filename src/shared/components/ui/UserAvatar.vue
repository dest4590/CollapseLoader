<template>
    <div
        class="avatar placeholder avatar-clickable"
        :class="{
            relative: showStatus,
            clickable: isClickable || !!resolvedSrc,
        }"
        @click="handleClick"
    >
        <div
            class="bg-base-100 text-primary-content rounded-full flex content-center overflow-hidden justify-center"
            :class="[
                sizeClasses,
                backgroundClass,
                { 'avatar-inner': !resolvedSrc },
            ]"
        >
            <img
            ...
            <span
                class="font-bold text-primary flex justify-center items-center avatar-text"
                :class="textSizeClass"
                v-else
            >
        ...
        <div
            v-if="showStatus && !globalUserStatus.isStreamer.value"
            class="absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 border-base-200 bg-base-200"
            :class="[statusClass, { 'status-indicator': isOnline }]"
        ></div>
    </div>
</template>
...
const resolvedSrc = computed<string | undefined>(() => {
    ...
    return (
        resolveApiAssetUrl(props.src || props.originalSrc || null) ?? undefined
    );
});
