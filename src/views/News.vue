...
const fetchNews = async () => {
    ...
    try {
        ...
        messageWraps.forEach((wrap) => {
            ...
            const photoElement = wrap.querySelector('.tgme_widget_message_photo_wrap');
            const videoElement = wrap.querySelector('.tgme_widget_message_video_player');

            if (dateElement && linkElement && (textElement || photoElement || videoElement)) {
                const href = linkElement.getAttribute('href') || '';
                const parts = href.split('/');
                const postIdStr = parts[parts.length - 1];
                const postId = parseInt(postIdStr, 10) || Math.floor(Math.random() * 1000000);
                ...
                if (videoElement) {
                    const thumbElement = videoElement.querySelector('.tgme_widget_message_video_thumb');
                    if (thumbElement) {
                ...
                if (textElement) {
                    ...
                    textHtml = textHtml.replace(/href="([^"]+)"/g, 'href="$1" target="_blank" rel="noopener noreferrer"');
                ...
                telegramNews.push({
                    ...
                    created_at: dateElement.getAttribute('datetime') || new Date().toISOString(),
                    updated_at: dateElement.getAttribute('datetime') || new Date().toISOString(),
                });
            }
        });

        if (telegramNews.length === 0 && htmlText) {
            throw new Error("Telegram parsing failed. HTML starts with: " + htmlText.substring(0, 150));
        }
