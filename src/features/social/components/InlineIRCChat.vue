<template>
    <div
        ...
        :class="
            isExpanded
                ? 'shadow-lg max-h-95'
                : 'shadow-sm max-h-17 hover:shadow-md'
        "
    >
        <button
            type="button"
            class="flex items-center justify-between w-full px-4 py-3 bg-base-300/40 cursor-pointer"
        ...
        >
            <div class="flex items-center gap-3">
                ...
                <div class="flex flex-col text-left leading-tight">
                    ...
                    <span
                        ...
                        :class="{
                            'w-[70%] truncate overflow-hidden':
                                latestActivity.length > 100,
                        }"
                    >
                        {{ latestActivity }}
                    </span>
            ...
            <div class="flex items-center gap-3">
                <button
                    class="p-1 rounded hover:bg-base-300/50 cursor-pointer"
                ...
                <ChevronDown
                    class="w-4 h-4 transition-transform duration-200 m-2"
        ...
        >
            <div class="flex flex-col h-70 bg-base-100/40">
                <div class="flex justify-between items-center">
                    ...
                    <transition name="irc-status">
                        ...
                        >
                            ...
                            >
                                <span
                                    class="w-2 h-2 rounded-full bg-blue-500/30 mr-1"
                                ></span>
                ...
                >
                    ...
                    >
                        ...
                        <template v-if="msg.sender">
                            ...
                            <span
                                v-for="(part, pIndex) in parseMessage(
                                    msg.content
                                )"
                                :key="pIndex"
                        ...
                        <template v-else>
                            <span
                                ...
                                :class="[
                                    {
                                        'cursor-pointer underline': part.isName,
                                        'font-bold': part.bold,
                                    },
                                ]"
                                @contextmenu.prevent="
                ...
                <transition name="irc-input">
                    <div
                        v-show="isExpanded"
                        class="p-4 bg-base-200 border-t border-base-300 flex gap-2"
                    >
                        <input
                            ref="ircInput"
                            ...
                            type="text"
                            :placeholder="$t('irc.inline.placeholder')"
    ...
    >
        <div
            class="bg-base-100 border border-base-300 rounded shadow-md p-2 flex flex-col context-menu"
        >
            <button
                class="text-left px-2 py-1 hover:bg-base-200 rounded flex items-center gap-2"
    ...
    if (type !== "system" && type !== "private") {
        ...
        if (nameStripRegex.test(msg)) {
            ...
            if (
                colonIndex !== -1 &&
                colonIndex < 50 &&
                !msg.toLowerCase().startsWith("system")
            ) {
...
const handleUsernameClick = async (sender?: {
    username?: string;
    userId?: string;
}) => {
...
const visibleMessages = computed(() =>
    messages.value.filter((m) => m && m.type !== "ping")
);
...
const latestActivity = computed(() => {
    ...
    if (!last) {
        return connected.value
            ? t("irc.inline.latest_activity_connected")
            : t("irc.inline.tap_to_connect");
    }
...
const statusMeta = computed(() => {
    switch (status.value) {
        case "connected":
            return {
                label: t("irc.inline.status.connected"),
        ...
        case "reconnecting":
            return {
                label: t("irc.inline.status.reconnecting"),
                textClass: "text-warning",
                icon: RefreshCw,
                iconClass: "animate-spin-slow text-warning",
            };
        case "connecting":
            return {
                label: t("irc.inline.status.connecting"),
                textClass: "text-info",
                icon: Loader2,
                iconClass: "animate-spin-slow text-info",
            };
        case "error":
            return {
                label: t("irc.inline.status.error"),
        ...
        default:
            return {
                label: t("irc.inline.status.disconnected"),
...
const sendMessage = async () => {
    ...
    try {
        ...
        addToast(t("irc.inline.send_failed"), "error");
    }
};
...
onUnmounted(() => {
    ...
    if (keydownHandlerRef)
        window.removeEventListener(
            "keydown",
            keydownHandlerRef as EventListener
        );
});
