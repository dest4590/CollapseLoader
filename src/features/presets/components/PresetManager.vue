        ...
        <div v-else>
            <div class="flex items-center justify-between mb-3">
                <div class="text-sm text-base-content/70">
                    {{
                        $t("theme.presets.pagination.page_of", {
                            current: currentPage,
                            total: totalPages,
                        })
                    }}
            ...
            <div
                ...
                :style="{
                    paddingBottom: selectedPresets.size > 0 ? '80px' : '0px',
                }"
            >
                <div
                    ...
                    :class="{
                        'ring ring-primary ring-offset-1 ring-offset-base-100 border-primary/60':
                ...
                >
                    <div class="card-body p-4">
                        ...
                        <div class="text-xs text-base-content/50">
                            {{
                                $t("theme.presets.created", {
                                    date: formatDate(preset.createdAt),
                                })
                            }}
                        </div>
        ...
        <transition name="slide-up-bottom">
            ...
            >
                <span class="font-medium text-xs sm:text-sm whitespace-nowrap">
                    {{
                        $t("theme.presets.selected_count", {
                ...
                <div class="flex items-center gap-1 sm:gap-2">
                    ...
                    <transition name="button-fade" mode="out-in">
                        <button
                            v-if="canDuplicateSelected"
                            @click="duplicateSelectedPresets"
                            :title="
                                $t('theme.presets.actions.duplicate_selected')
                            "
                            class="btn btn-sm btn-ghost hover:bg-neutral-focus p-2 aspect-square"
                        >
...
const deleteSelectedPresets = async () => {
    ...
    await loadPresets();
    addToast(
        t("theme.presets.messages.multiple_deleted", { count: items.length }),
        "success"
    );
    clearSelection();
};
...
const duplicateSelectedPresets = async () => {
    ...
    await loadPresets();
    addToast(
        t("theme.presets.messages.multiple_duplicated", {
            count: items.length,
        }),
        "success"
    );
...
const exportSelectedPresets = async () => {
    try {
        ...
        await navigator.clipboard.writeText(json);
        addToast(
            t("theme.presets.messages.multiple_exported", {
