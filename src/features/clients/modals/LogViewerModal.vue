<template>
    <div class="flex flex-col h-full max-h-[80vh]">
        <div
            class="flex flex-col gap-4 pb-4 border-b border-base-300/30 shrink-0"
        >
            <div class="flex flex-wrap items-center justify-between gap-4">
                <div class="flex items-center gap-4">
                    ...
                    <div class="join">
                        <div
                            class="join-item flex items-center px-3 bg-base-200 border border-base-300"
                        >
            ...
            <div class="flex flex-wrap items-center gap-2">
                <span class="text-xs font-semibold opacity-50 uppercase mr-2"
                    >{{ t("appLogs.filterByLevel") }}:</span
                >
                <button
        ...
        >
            ...
            >
                {{
                    searchQuery || selectedLevels.length < levels.length
                        ? t("appLogs.noMatchingLogs")
                        : t("appLogs.noLogs")
                }}
            </div>
...
const filteredLogs = computed(() => {
    ...
    return logs.value.filter((log) => {
        ...
        if (logHasLevel) {
            return (
                matchesSearch && selectedLevels.value.includes(logHasLevel.id)
            );
        }
