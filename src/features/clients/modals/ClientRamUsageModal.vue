<template>
    <div class="space-y-5">
        <div
            class="rounded-2xl border border-base-content/10 bg-base-200/70 p-4"
        >
            <div class="flex flex-wrap items-start justify-between gap-4">
                ...
                <div
                    ...
                    :class="
                        ramUsage?.is_running ? 'badge-success' : 'badge-warning'
                    "
                >
                    <span
            ...
            <div class="mt-4 grid gap-3 md:grid-cols-3">
                ...
                >
                    <p
                        class="text-xs uppercase tracking-[0.2em] text-base-content/40"
                    >
                    ...
                    <p class="mt-2 text-3xl font-black text-primary">
                        {{
                            ramUsage
                                ? clientRamService.formatMiB(
                                      ramUsage.total_memory_mib
                                  )
                                : "--"
                        }}
                    </p>
                    <p class="mt-1 text-sm text-base-content/60">
                        {{
                            ramUsage
                                ? clientRamService.formatBytes(
                                      ramUsage.total_memory_bytes
                                  )
                                : "Waiting for data"
                        }}
                    </p>
                ...
                >
                    ...
                    <p class="mt-2 text-3xl font-black text-secondary">
                        {{
                            ramUsage
                                ? clientRamService.formatPercent(
                                      ramUsage.system_memory_percent
                                  )
                                : "--"
                        }}
                    </p>
                    <p class="mt-1 text-sm text-base-content/60">
                        ...
                        {{
                            ramUsage
                                ? clientRamService.formatMiB(
                                      ramUsage.system_total_memory_mib
                                  )
                                : "system memory"
                        }}
                    </p>
            ...
            <div class="mt-4 space-y-2">
                <div
                    class="flex items-center justify-between text-xs uppercase tracking-[0.18em] text-base-content/40"
                >
        ...
        <div
            class="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-base-content/10 bg-base-200/50 px-4 py-3"
        >
            <div class="text-sm text-base-content/70">
                <p class="font-semibold text-base-content">
                    {{ t("modals.client_ram_usage.last_refresh") }}:
                    {{ lastUpdatedLabel }}
                </p>
                <p>
...
const refresh = async () => {
    ...
    try {
        ramUsage.value = await clientRamService.getClientRamUsage(
            props.clientId
        );
        ...
        errorMessage.value =
            "Unable to read RAM usage for this client right now.";
    } finally {
