...
class SyncService {
    ...
    private isAutoGeneratedAccount(account: any): boolean {
        if (!account || !Array.isArray(account.tags)) return false;
        return account.tags.includes("auto-generated");
    }
        ...
        try {
            ...
            if (accountsToSync.length !== (accounts?.length ?? 0)) {
                console.debug(
                    ...
                    accounts.length - accountsToSync.length
                );
            }
        ...
        if (cloudData.accounts_data && Array.isArray(cloudData.accounts_data)) {
            try {
                ...
                for (const cloudAccount of cloudData.accounts_data) {
                    if (this.isAutoGeneratedAccount(cloudAccount)) {
                        continue;
                    }
