<template>
    <div class="flex flex-col h-full max-h-[80vh]">
        <div class="text-center space-y-4 shrink-0 pb-4">
            <div class="space-y-2">
                <div class="flex items-center justify-center gap-4 text-sm">
                    <div
                        class="flex items-center gap-2 px-3 py-1 bg-base-200 rounded-full"
                    >
                        <span class="text-base-content/60"
                            >{{ t("updater.current") }}:</span
                        >
                        <code class="text-primary font-mono"
                            >v{{ updateInfo.current_version }}</code
                        >
                    </div>
                    <ChevronRight class="w-4 h-4 text-base-content/40" />
                    <div
                        class="flex items-center gap-2 px-3 py-1 bg-primary/10 rounded-full"
                    >
                        <span class="text-base-content/60"
                            >{{ t("updater.latest") }}:</span
                        >
                        <code class="text-primary font-mono font-bold">{{
        ...
        <div
            class="flex-1 overflow-y-auto min-h-0 space-y-6"
            style="max-height: calc(80vh - 160px)"
        >
            <div
                v-if="updateInfo.changelog && updateInfo.changelog.length > 0"
                class="space-y-4"
            >
                <div class="divider">
                    <h4
                        class="font-semibold flex items-center gap-2 text-primary"
                    >
                ...
                <div class="space-y-6">
                    <div
                        v-for="(entry, index) in displayedChangelogEntries"
                        :key="entry.version"
                        class="relative timeline-entry"
                        :class="{ 'opacity-60': index > 0 }"
                    >
                        <div
                            v-if="index < displayedChangelogEntries.length - 1"
                        ...
                        <div class="flex items-center gap-4 mb-4">
                            <div class="flex-1">
                                ...
                                >
                                    <span
                                        v-for="highlight in entry.highlights.slice(
                                            0,
                                            3
                                        )"
                                        :key="highlight"
                                    ...
                                    <span
                                        v-if="entry.highlights.length > 3"
                        ...
                        <div class="ml-16 space-y-3">
                            ...
                            >
                                ...
                                <div class="space-y-2">
                                    <div
                                        v-for="change in categoryGroup"
                                        :key="change.description_key"
                    ...
                    <div
                        v-if="updateInfo.changelog.length > maxDisplayedEntries"
                        class="text-center pb-4"
                    >
                        <button
                            @click="toggleShowAllChangelog"
